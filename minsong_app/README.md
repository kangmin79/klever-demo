...
## 8/12 밤 — v0.7: 홈을 참나루 웹 그대로 (사장님 지시)
- 사장님: "앱의 인증 카드는 일단 삭제하고 웹을 그대로 옮겨보자"
- **홈에서 인증 미션카드 제거** — 인증 탭(202권+진행판)은 그대로. CertCard 컴포넌트도 인증 탭에서 계속 사용
- **크림 바탕(#faf8f3) 복귀** + **세리프 히어로**: 웹과 같은 library_sections hero 칸(✦ 오늘의 사서 추천 —
  세리프 대제목·저자·"사서 한마디"·자세히 보기·표지). hero 칸이 비어 있으면 전자책 오늘의 추천으로 폴백
- 새 패키지 expo-font + @expo-google-fonts/noto-serif-kr (c:\tmp 재설치 시 포함)
- 관리자 '우리 도서관'의 hero 칸에 책을 담고 부제를 적으면 웹·앱 히어로가 동시에 바뀜

## 8/12 밤 2 — v0.8: 웹 상단 카테고리 5개 이식
- 홈 검색 아래 가로 칩: **우리 도서관 / 독서 챌린지 / 세계고전 / International / 커뮤니티** (웹 탭 그대로)
- 우리 도서관=기존 홈 본문. 세계고전·International=**각 영역의 library_sections**(hero+선반, 고전 책은
  gb-/kr- id → 북스타 표지·바로 읽기 상세). 챌린지=**library_programs**(발행 0건이면 웹과 같은 빈 안내).
  커뮤니티=**community_posts** 공지·행사·자유 읽기(터치로 본문 펼침)
- 전부 웹과 같은 테이블 — 사서가 관리자에서 바꾸면 웹·앱 동시 반영
