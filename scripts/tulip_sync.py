...
# ---------- 표지 (P2) ----------
# 🔴 이 저장소는 **공개**다. 새 키를 여기 적지 말 것 — 환경변수 ALADIN_TTBKEY로 넣는다.
#   (2026-08-13 설정: 사용자 환경변수에 등록 URL이 bookstar.co.kr인 계정 키를 넣어둠.
#    알라딘 약관이 문제 삼는 건 명의보다 **등록 URL과 실제 사용처의 일치**라 그쪽이 맞다.
#    ⚠️두 키를 동시에 돌려 하루 10,000회 쓰는 건 한도 우회 — 하나만 쓴다.)
#   아래 기본값은 **이미 이 공개 저장소에 노출된 옛 키**다(새로 노출되는 것 없음).
#   환경변수가 없을 때 조용히 죽지 않도록 남겨둔 폴백일 뿐, 정리 대상이다.
